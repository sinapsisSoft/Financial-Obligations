...
// function setDataProvider(dataSetCredit) {
//   try {
//     loadPageView();
//     dataSetCredit = '{"POST":"POST",' + dataSetCredit + "}";
...
//**Function get Credit **/
function getDataCredit(table, dataSetCredit, typeSend, user) {
  try {
    loadPageView();
    var xhttp = new XMLHttpRequest();
    var arrayCell = new Array("Consecutivo","Fecha Solicitud", "Identificación", "Solicitante", "Estado", "Ver", "Desc.", "Seg.");
    var JsonData;
    xhttp.open("POST", "../../php/bo/bo_creditForm.php", true);
    xhttp.setRequestHeader("Content-Type", "application/json; charset=UTF-8");
    xhttp.onreadystatechange = function () {
      ...
      if (this.readyState === 4 && this.status === 200) {
        ...
        if (json.length != 0) {
          if (typeSend == 1) {
            tableCredit = new Table(table, arrayCell, json);
            tableCredit.createtableCredits();               
            enableScroll();         
          }
          else if (typeSend == 0) {
            setDataForm(json);
    ...
    if (typeSend == 0) {
      JsonData = '{"GET":"GET","Cre_id":"' + dataSetCredit + '"}';
    }
    if (typeSend == 1) {
      JsonData = '{"GET":"GET_SEARCH","Cre_name":"' + dataSetCredit + '","User_id":"' + user +'"}';
    }
...
function loadView() {
  loadPageView();  
  getDataCredit("tableCredit", "", 1, getUserid());
  getActionStorage();
}
//************ LOAD VIEW STORAGE ******************/
...
//************SEARCH PROVIDER**************//
function searchCredits(e) {
  try {
    var objForm = document.getElementById('formsearchCredits');
    let intLogForm = objForm.querySelectorAll('input').length;
    let jsonData = '';
    for (let i = 0; i < intLogForm; i++) {
      jsonData = objForm[i].value;
    }
    getDataCredit("tableCredit", jsonData, 1, getUserid());
  }
  catch (error) {
    console.error(error);
  }
...
function viewModalCreSecurity(Cre_id){
  try {
    loadPageView();
    var xhttp = new XMLHttpRequest();
    ...
    xhttp.onreadystatechange = function () {
      ...
      if (this.readyState === 4 && this.status === 200) {
        var json = JSON.parse(xhttp.responseText);
        if (json.length != 0) {
            tableCredit = new Table("tableCreditSecurity", arrayCell, json);
            tableCredit.createtableCreditSecurity();               
            enableScroll();         
            $('#securityModalCenter').modal('show');
        }
      }
    };
    JsonData = '{"GET":"GET_SECURITY","Cre_id":"' + Cre_id + '"}';    
    xhttp.send(JsonData);
  } catch (error) {
    console.error(error);
    enableScroll();
...
function setStatusCredit(dataSetCredit) {
  try {
    loadPageView();
    dataSetCredit = '{"POST":"STATUS",' + dataSetCredit + "}";
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST", "../../php/bo/bo_creditForm.php", true);
    xhttp.setRequestHeader("Content-Type", "application/json; charset=UTF-8");
    xhttp.onreadystatechange = function () {
...
function finalStatusCredit(id,idStatus,idSelect){
  let status = "";
  if (idStatus == 11) {
    status = "Revisado";
    let message = confirm("¿Está seguro que desea cambiar al estado "+status+"?");
    if (message == true) {
      jsonStatus = '"Cre_id":"'+id+'","Stat_id":"'+idStatus+'"';
      setStatusCredit(jsonStatus);
      loadView();
    }
    else {
